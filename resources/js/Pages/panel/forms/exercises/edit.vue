        ...
        <v-sheet>
            ...
            <div class="grid grid-cols-12 gap-2">
                ...
                <div class="col-span-12 xl:col-span-8 lg:col-span-8 px-4">
                    ...
                    <template v-else>
                        <div class="text-center">تمرینی اضافه نشده است</div>
                    </template>
                </div>
...
const fetchData = async () => {
    ...
    const { data: program } = await ApiService.get(
        `/api/panel/form/${route.params.id}/programs/${route.params.exercise}`
    );
    console.log("program", program);

    const selected_class = classes.value.find(
        (item, index) => item.value == program.data.class
    );

    form.value.class = selected_class;

    program.data.movements.map((item, index) => {
        let selected_type = types.value.find(
            (type_item, index) => type_item.value == item.type
        );
        let formated_movement = { type: selected_type, movement: [] };
        ...
        if (selected_type.value == "1") {
            ...
            created_movements.value.push(formated_movement);
